...
namespace OneDirect.Repository
{
    public class DeviceCalibrationRepository : IDeviceCalibrationInterface
    {
        ...
        public void deleteDeviceCalibrationCascade(string setupId)
        {
        ...
        {
            //var device = (from p in context.DeviceCalibration
            //             where p.SetupId == pDeviceCalibration.SetupId
            //             select p).FirstOrDefault();
            //if (device != null)
            //{
            //    context.Entry(device).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
            //    context.SaveChanges();
            //}
