    ...
    public class AppointmentScheduleRepository : IAppointmentScheduleInterface
    {
            ...
            try
            {
                var appointments = (from p in context.AppointmentSchedule
                                    ...
                                    select new AppointmentScheduleView
                                    {
                                        ...
                                        UserID = p.UserId,
                                        UserName = u.Name,
                                        DateTime = p.Datetime.ToString("MM/dd/yyyy hh:mm:ss tt"),
        ...
        public string NoShowAppointments(int patientId, string userId, string datetime)
        {
            try
            {
                var appointments = (from p in context.AppointmentSchedule
                                    where p.PatientId == patientId && p.UserId == userId
                                    && p.Datetime >= Convert.ToDateTime(datetime) && p.Datetime.AddHours(1) < DateTime.Now
                                    select p).ToList();
                if (appointments != null && appointments.Count > 0)
                {
                    ...
                    //appointments.Select(x => x.CallStatus = "Caller No Show");
                    //context.Entry(appointments).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                    //context.SaveChanges();
                }
        ...
        public string InsertAvailability(string userId, string userType, string timezoneoffset, string day, string hour)
        {
            try
            {
                ...
                else
                {
                    ...
                    lavailability.TimeZoneOffset = TimeZoneInfo.Local.Id;// TimeZoneInfo.Local.GetUtcOffset(DateTime.UtcNow).TotalMinutes.ToString();
                    //lavailability.Time
                    ...
                    foreach (Availability avail in lavailabilityList)
                    {
                        ...
                        foreach (string hour in hourofDay)
                        {
                            ...
                            if (!string.IsNullOrEmpty(datetime))
                            {
                                ...
                                //availability.end = (Convert.ToDateTime(datetime).Hour + 1).ToString() + ":00";
            ...
            try
            {
                if (code == 1)
                {
                    ...
                    if (lavailabilityList != null && lavailabilityList.Count > 0)
                    {
                        ...
                        //var dayofWeek = string.Join(",", lavailabilityList
                ...
                if (lavailabilityList != null && lavailabilityList.Count > 0)
                {
                    ...
                    if (result != null && result.Count > 0)
                    {
                        ...
                        if (lappointmentScheduleList != null && lappointmentScheduleList.Count > 0)
                        {
                            ...
                            {
                                //x.Datetime = Convert.ToDateTime(Utilities.ConverOneTimeZonetoAnotherTimeZone(x.Datetime, TimeZoneInfo.Local.GetUtcOffset(DateTime.UtcNow).TotalMinutes.ToString(), timezoneoffset));
